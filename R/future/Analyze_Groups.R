...
subjID <- "SUBID"
inclusionName <- "INCLUSION_PR_MatchedGroup"
covsFormula <- "~Enrollment_Group_NC_MDD_MALTXMDD"
subjDataName <- "/dfs3/som/rao_col/maltx/datasets/redcap/n93_Demo+Clinical+Neuropsych_20190601.rds"
inputPath <- "/dfs3/som/rao_col/maltx/datasets/freesurfer/n86_PR_COMBINED_20190610.csv"
OutDirRoot <- "/dfs3/som/rao_col/maltx/analysis/sandbox/rjirsara/Results"
...
m <- mclapply(model.formula, function(x) {
  ...
  results <- lm(formula = x, data=dataSubj)
  summary <- Anova(results)
  residuals <- results$residuals
  missing <- as.numeric(results$na.action)
...
outsubDir <- paste0("aov_formula_",outName)
