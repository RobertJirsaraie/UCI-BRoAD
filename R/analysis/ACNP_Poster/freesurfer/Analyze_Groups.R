...
subjDataName <- "/dfs1/som/rao_col/maltx/datasets/clinical/n50_Demo+MDD+MAL_20181203.rds"
inputPath <- "/dfs1/som/rao_col/maltx/datasets/neuroimaging/n38_FreeSurf_ASEG_Vol_20181107.csv"
OutDirRoot <- "/dfs1/som/rao_col/sandbox/jirsaraie/MALTX/results/ACNP_Poster"
inclusionName <- "QA_T1w_Exclude"
subjID <- "SUBID"
covsFormula <- "~Age+Sex+EnrollmentGroups"
pAdjustMethod <- "fdr"
ncores <- 5
residualMap <- FALSE
...
m <- mclapply(model.formula, function(x) {
  ...
  foo <- aov(formula = x, data=dataSubj, method="REML")
  summary <- summary(foo)
