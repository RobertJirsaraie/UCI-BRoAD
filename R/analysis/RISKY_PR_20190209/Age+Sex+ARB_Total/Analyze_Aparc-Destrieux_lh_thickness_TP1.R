...
subjDataName <- "/dfs3/som/rao_col/risky/datasets/redcap/n53_Demo+Clinical_TP1_20190206.rds"
inputPath <- "/dfs3/som/rao_col/risky/datasets/freesurfer/n46_Aparc-Destrieux_lh_thickness_20190205.csv"
OutDirRoot <- "/dfs3/som/rao_col/risky/analysis/sandbox/jirsaraie/ProgressReport_20190205"
inclusionName <- "Exclusions"
subjID <- "SUBID"
covsFormula <- "~s(age_years,k=4)+sex+arb_total"
pAdjustMethod <- "fdr"
ncores <- 5
residualMap <- FALSE
...
if (is.null(m[[1]]$s.table)) {
  ...
  output<-output[order(output$tval.arb_total),]
