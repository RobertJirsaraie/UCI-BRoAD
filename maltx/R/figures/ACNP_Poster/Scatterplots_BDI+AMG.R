...
CSV<-read.csv("/dfs1/som/rao_col/maltx/datasets/neuroimaging/n38_FreeSurf_ASEG_Vol_20181107.csv")
...
ResponceVars <- names(DATA)[grep("Left.Amygdala",names(DATA))]

Models <- lapply(ResponceVars, function(x) {
  gam(substitute(i ~ s(Age,k=4) + Sex + BDI_SS_Total, list(i = as.name(x))), method="REML", data = DATA)
})
...
plotdata <- visreg(Models[[1]],'BDI_SS_Total',type = "conditional",scale = "linear", plot = FALSE)
...
Scatterplot<-ggplot() +
  ...
  geom_line(data = smooths, aes(x = x, y=lower), linetype="dashed", colour = Color, alpha = 0.9, size = 2.75) +
  geom_line(data = smooths, aes(x = x, y=upper), linetype="dashed",colour = Color, alpha = 0.9, size = 2.75) +
...
ggsave(file="/dfs1/som/rao_col/sandbox/jirsaraie/MALTX/figures/ACNP_Poster/ScatterPlot_BDI_AMG.pdf",device = "pdf",width = 12, height = 9, units = c("in"))
