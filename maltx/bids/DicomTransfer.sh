...
if [ ! -f /dfs1/som/rao_col/maltx/bids/sub-${SubjectID}x${ValidateID}/ses-1/anat/sub-${SubjectID}x${ValidateID}_ses-1_*.nii.gz ]; then

echo 'This subject did not have their Niftis Uploaded Properly as of this date:' $TODAY 'Check on Xnat and with Derek to troubleshoot the issue!' > /dfs1/som/rao_col/maltx/scripts/bids/logs/${SubjectID}x${ValidateID}_MissingBIDs.txt
rm /dfs1/som/rao_col/maltx/dicoms/${SubjectID}_${ValidateID}.zip
rm -rf /dfs1/som/rao_col/maltx/dicoms/${SubjectID}_${ValidateID}

	else

rm /dfs1/som/rao_col/maltx/scripts/bids/logs/${SubjectID}x${ValidateID}_MissingBIDs.txt
